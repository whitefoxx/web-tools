...
| the popup or the settings page                    | `store-listing.md` wherever it describes the surfaces — the "what you'll see" line, the `userScripts` justification, the site-script bullet — and `render.mjs`'s Get-started steps. **No screenshot depicts either page**; if you add one it will rot fastest. Added 2026-09-21, when three listing lines still said site scripts were listed "in the popup" and they have never been | nothing type-checks a listing, and the half a reviewer reads is the half worth auditing first                                                                           |
