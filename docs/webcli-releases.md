...
| ---------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| always (before pasting anything)                                       | `npm run check:listings` — it is part of `npm run check`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | a listing is a PASTE SOURCE, not a rendered document: `**bold**` ships literally, `>` has to be deleted by hand, and a hard-wrapped line pastes as a mid-sentence break |
...
| popup or settings UI                                                   | `store-listing.md` wherever it describes the surfaces (the "before you install" note, the single-purpose description, the `userScripts` justification) + `render.mjs` screenshot 5's step 1. **No screenshot depicts either page** (they are Hero / How it works / Toolbelt / Why / Get started). Corrected 2026-08-07: this row used to say "screenshot 5 if it depicts the popup", which reads as if it does, and cost a wrong entry in a release checklist. Extended 2026-09-21, when 0.4.0 grew a settings page and four lines of listing copy still said the popup held the catalog and the site scripts — two of them in text the REVIEWER reads | if you ever add one, it rots fastest — the popup changed in 0.2.0, 0.3.0 and 0.4.0                                                                                      |
...
## 4c. `npm run check` was never green — fixed 2026-09-21

Step 1 above has said "`npm run check` must be green" since this file existed,
and it never was. `typecheck` and `test` passed; the other two halves did not:

- **lint** — one error, `@typescript-eslint/no-this-alias` on `MiniNode.closest`,
  present since the very first commit (`2a4a50a`, the base extraction). The rule
  targets `const self = this` captured by a closure; this is a loop cursor
  (`for (let n: MiniNode | null = this; n; n = n.parentNode)`), which has to start
  at the node itself because `closest` can match self. Suppressed at the line with
  that reason written next to it, not silenced in the config.
- **format:check** — 65 files. The repo inherited web-agent's formatting drift at
  extraction and nothing ever swept it, so a real failure was indistinguishable
  from the standing noise. Swept with `prettier --write`, behind a new
  `.prettierignore`.

**What `.prettierignore` holds, and why each entry is there** — Prettier has no
business rewriting a file a script generates or a human hand-aligned:

| Ignored                    | Reason                                                              |
| -------------------------- | ------------------------------------------------------------------- |
| `pnpm-lock.yaml`           | generated                                                           |
| `store/*/promo.html`       | generated by `render.mjs`                                           |
| `store/*/svg/`, `images/`  | generated by `render.mjs` / `raster.mjs`                            |
| `store/*/store-listing.md` | paste targets — exact intra-line spacing IS the deliverable (below) |

That last one is the load-bearing entry. `store-listing.md` is copy pasted into
the Chrome Web Store dashboard field by field, and Prettier collapses the
alignment inside a line — it wanted to turn WebCLI's

```
2. Ask your agent to run:  npx skills add whitefoxx/web-tools -g
```

into a single space. Formatting a paste target edits the product.

**Proving a formatting sweep changed nothing.** 63 files moved right before a
store upload, so "the tests pass" is not enough on its own. Minify the bundle
before and after and compare — a minifier normalizes exactly what Prettier
changes (whitespace, trailing commas, redundant grouping parens), so an identical
result means zero program tokens moved:

```bash
npx esbuild --minify --format=esm before.js > a.min.js
npx esbuild --minify --format=esm dist-webcli/assets/webcli-service-worker.ts-*.js > b.min.js
# normalize the content-hashed import specifier (register-<hash>.js), then diff
```

For the 2026-09-21 sweep both sides minified to **204,975 bytes, identical**.

**Lesson.** A check that has never passed is not a check; it is noise that trains
everyone to skip the step. Either make it green or take it out of `check` — a
release checklist whose first gate is known-red teaches you to walk past gates.
...
**And a settings page** (added 2026-09-21, before this version was ever
uploaded). The popup was carrying five jobs in 320px and two of them had
outgrown it: a site script is a rule that runs on your pages until you remove
it, and deciding that means reading its matches and its source — the popup could
show a name and a checkbox. `manifest.webcli.json` gains `options_ui`
(`open_in_tab: true`, not a permission, so no new justification); the page is
Connection / Tools / Site scripts, and the popup keeps the connection plus a
count linking into each. Full rationale in [webcli.md](./webcli.md) §18.

Listing copy swept again for it: four lines still said the popup held the
catalog and the site scripts, **two of them in text the reviewer reads** (the
single-purpose description and the `userScripts` justification). Screenshot 5's
step 1 re-rendered and re-rastered — the replacement line is 81 chars against
the 83 that already ships, so it cannot overflow a width the current image
clears.
...
> **Version note.** 0.4.0 was packed on 2026-09-21 and then held rather than
> uploaded, so the number is still free — a version burns only on upload. The
> settings page went into this release instead of a 0.5.0 because the one
> uncertain thing here is the `userScripts` review, and pairing it with a pure
> UI addition costs nothing: both are in the same package either way.
